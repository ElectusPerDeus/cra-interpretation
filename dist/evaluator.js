...
import { CRA_INTERPRETATION_DOMAINS } from "./contracts.js";
...
const canonicalQuestionOptionIndex = buildQuestionOptionIndexFromManifest(manifestData);
function asManifest(input) {
    return input;
}
...
function hasRequiredTopLevelFields(manifest) {
    return Boolean(manifest.manifestId &&
...
function isRuleShapeValid(rule) {
    return Boolean(rule.interpretationId &&
        rule.version &&
        rule.scope &&
...
function evaluatePredicate(predicate, input) {
...
function dedupeAndSortServiceTags(serviceTags) {
    const filtered = serviceTags.filter((tag) => allowedServiceTags.includes(tag));
    return Array.from(new Set(filtered)).sort((a, b) => a.localeCompare(b));
}
function triggeredAtomicRules(manifest, input) {
    return manifest.rules.atomic.filter((rule) => rule.enabled && Boolean(rule.questionId && rule.optionId && input.answersJson[rule.questionId] === rule.optionId));
}
function triggeredCompositeRules(manifest, input) {
    return manifest.rules.composite.filter((rule) => rule.enabled && rule.predicates.every((predicate) => evaluatePredicate(predicate, input)));
}
function triggeredDomainRollups(manifest, input) {
    return manifest.rules.domainRollup.filter((rule) => rule.enabled && rule.predicates.every((predicate) => evaluatePredicate(predicate, input)));
}
function buildQuestionOptionIndexFromManifest(manifest) {
    const index = new Map();
    const add = (questionId, optionId) => {
        const options = index.get(questionId) ?? new Set();
...
export function getCraInterpretationManifest() {
    return asManifest(manifestData);
}
export function validateManifestSchemaShape(manifest) {
    const errors = [];
...
export function validateServiceTags(manifest) {
...
export function validateManifestReferences(manifest) {
    ...
    const knownDomains = new Set(CRA_INTERPRETATION_DOMAINS);
    for (const rule of manifest.rules.atomic) {
            ...
            if ((predicate.type === "answer_equals" || predicate.type === "answer_in") && predicate.questionId) {
                ...
                if (predicate.type === "answer_in") {
                    const values = Array.isArray(predicate.value)
                        ? predicate.value.filter((entry) => typeof entry === "string")
                        : [];
                    for (const value of values) {
                        if (!optionSet.has(value)) {
                            errors.push(`orphaned_predicate_option:${rule.interpretationId}:${predicate.questionId}:${value}`);
...
export function evaluateCraInterpretation(input, manifest = getCraInterpretationManifest()) {
    ...
    const suppressedTopicKeys = new Set(composite
