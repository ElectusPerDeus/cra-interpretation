...
function allQuestionOptionKeys(manifest) {
    const keys = new Set();
...
function optionKeysCovered(manifest) {
    const keys = new Set();
    for (const rule of manifest.rules.atomic) {
...
function collectAllDomains(manifest) {
...
function collectDomainsWithPositivePath(manifest) {
    const domains = new Set();
    const allRules = [
        ...manifest.rules.atomic,
        ...manifest.rules.composite,
...
export function buildCoverageMap(manifest = manifestData) {
